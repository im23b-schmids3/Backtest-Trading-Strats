# HTFLevelLiquidityFVG.BTC_LONG_SHORT_V2_RELAXED_MSS

## Seal, Scope, and V1 Evidence

This is the sealed, immutable specification for `HTFLevelLiquidityFVG.BTC_LONG_SHORT_V2_RELAXED_MSS`. It is a causal BTC long/short study specification. No optimization, parameter search, adaptive thresholding, look-ahead, retry after a terminal disposition, or cross-candidate leakage is permitted. A change requires a new versioned specification and SHA-256.

The read-only V1 funnel diagnostic was read before sealing. Its observed evidence, per V1 candidate, was 186 proposed setups, 4 MSS confirmations, 4 displacement passes, 3 valid FVGs, and zero projected-RR passes, orders, or trades. Terminal counts were `MSS_WINDOW_EXPIRED=141`, `SUPERSEDED_BY_NEW_SWEEP=21`, `DISPLACEMENT_REJECTED=18`, `PROJECTED_RR_REJECTED=3`, `BIAS_INVALIDATED_BEFORE_ENTRY=2`, and `FVG_TOO_SMALL=1`. The diagnostic's earliest measured bottleneck was MSS confirmation (186 -> 4 per candidate). This evidence is solely the motivation for this sealed V2 design; it is not permission for further tuning.

V2 has exactly three behavioral differences from V1: the MSS window is 24 completed 5-minute bars rather than 12; the confirmed 5-minute MSS swing uses exactly two completed bars on the left and one completed bar on the right rather than the V1 two-sided fractal; and the candidate registry is the three V2 minimum projected entry-to-TP2 thresholds stated below. Every other rule, gate, causal rule, terminal disposition, event scope/reconciliation rule, artifact layout, phase lock, and Phase-B lock remains unchanged in meaning.

Anti-overfitting prohibitions: no other parameter changes; no alternate MSS windows; no alternate swing definitions; no FVG, stop, or gate changes; no iterative Phase-A retuning; and no Phase-B access. No code, market-data operation, Phase A, Phase B, Alpha, candidate study, or research artifact operation happened while sealing this specification.
...
For each setup, inspect only later confirmed 5-minute bars. A confirmed directional swing high is a bar whose high is strictly above the highs of exactly two completed bars immediately to its left and greater than or equal to the high of exactly one completed bar immediately to its right. A confirmed directional swing low is a bar whose low is strictly below the lows of exactly two completed bars immediately to its left and less than or equal to the low of exactly one completed bar immediately to its right. A directional swing confirms only after that one right bar closes.

- Within 24 completed 5-minute bars after the sweep bar, long requires a close above the most recent confirmed qualifying swing high that existed before the MSS close; short requires a close below the most recent confirmed qualifying swing low that existed before the MSS close. The right bar confirming that swing must have closed before the MSS close. This is MSS. No future information is permitted.
...
- Failure to confirm MSS, displacement, and FVG in the stated order inside the 24-bar window is `MSS_WINDOW_EXPIRED`.
...
|---|---:|---|
| `HTFLFVG-V2-MIN1P5R` | >= 1.5R | admitted |
| `HTFLFVG-V2-MIN2P0R` | >= 2.0R | admitted |
| `HTFLFVG-V2-MIN2P5R` | >= 2.5R | admitted |
...
3. On subsequent completed 5-minute bars, within 24 completed bars after the sweep, require a bullish MSS: close above the most recent confirmed qualifying swing high that existed before the MSS close, with exactly two completed left bars and one completed right bar and with that right bar closed before the MSS close. Then require bullish displacement and the first valid bullish FVG within two completed bars after MSS and within the 24-bar post-sweep window.
...
3. On subsequent completed 5-minute bars, within 24 completed bars after the sweep, require a bearish MSS: close below the most recent confirmed qualifying swing low that existed before the MSS close, with exactly two completed left bars and one completed right bar and with that right bar closed before the MSS close. Then require bearish displacement and the first valid bearish FVG within two completed bars after MSS and within the 24-bar post-sweep window.
...
Emit append-only audit records for aggregation confirmation, bias, level lifecycle, sweep, MSS, displacement, FVG, gate transitions, order state, fill, exit, disposition, and reconciliation. Every setup record stores the source bars and snapshots that supported its decision: daily EMA50 and its five-day slope, selected level ID/price/confirmation, 15m prior ATR and wick calculation, pre-MSS directional swing identity including its two left bars and one right bar, the right-bar close time, 5m prior ATR, FVG three bars/boundaries/width, candidate projected R, entry/stop/TP1/TP2, quantity and cost assumptions, and terminal/exit reason.
...
- Synthetic causal tests prove long and short directional swings require exactly two completed left bars and one completed right bar; prove that the right bar closes before the MSS close; and reject an otherwise matching MSS that relies on a future bar.
- Boundary tests prove the MSS window admits the 24th completed post-sweep 5-minute bar and rejects the 25th, including failure ordering for MSS, displacement, and FVG.
- Long and short tests prove the most recent qualifying confirmed swing existing before the MSS close is used, with strict directional break inequalities and correct mirror behavior.
- Regression tests prove every unchanged sweep, displacement, FVG, entry, stop, target, partial-exit, same-bar ambiguity, session, holding, costs, quantization, candidate-isolation, terminal-disposition, artifact-integrity, and reconciliation rule.
- Registry tests prove only the three sealed V2 candidate IDs exist and that their sole differing field is minimum projected entry-to-TP2 R: 1.5R, 2.0R, and 2.5R.
...
- confirmed-only directional-swing availability using exactly two completed left bars and one completed right bar, right-bar closure before MSS close, prior-only ATR/EMA inputs, and no future information;
...
- the required synthetic causality, long/short, MSS-window boundary, partial-exit, same-bar ambiguity, time-control, and artifact-integrity regression tests before any real run.
