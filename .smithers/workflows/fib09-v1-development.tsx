...
export default smithers((ctx) => (
  <Workflow name="fib09-v1-development">
    <Task id="deterministic-development" output={outputs.output} agent={{ kind: "shell" } as never}>
      {`python -m research_pipeline fib09-v1-development --eth-manifest ${ctx.input.ethManifest} --btc-manifest ${ctx.input.btcManifest} --artifact-root ${ctx.input.artifactRoot} --repository-root ${ctx.input.repositoryRoot}`}
    </Task>
  </Workflow>
));
