...
from ..value_area_trap.data import AggregateTradeManifest, MonthlyAggregateTradeManifest, PARQUET_SCHEMA
from ..value_area_trap.profile import (
    FiveMinuteBar,
    NY,
    UTC,
    UTC_SESSION_LABEL,
    US_CASH_SESSION_LABEL,
    US_CASH_SESSION_HOLIDAYS_2026,
    is_us_cash_trading_day,
)
...
class ValueAreaTrapAdapter:
    ...
    def __init__(self, specification: StrategySpec, repository_root: str | Path = ".", manifest_path: str | Path | None = None):
        ...
        self.identity = AdapterIdentity(
            ...
            adapter_version=(
                "value-area-trap-3"
                if specification.baseline_parameters.get("session_definition") != "US_CASH_WINDOW_PROXY"
                else "value-area-trap-2"
            ),
        )
        self.capabilities = AdapterCapabilities(
            parameter_experiment=False,
            supported_markets=[str(specification.baseline_parameters.get("symbol", "BTCUSDT"))],
    ...
    @staticmethod
    def _manifest_digest(manifest: AggregateTradeManifest | MonthlyAggregateTradeManifest) -> str:
        raw = manifest.model_dump(mode="json")
        raw.pop("manifest_hash", None)
        return hashlib.sha256(json.dumps(raw, sort_keys=True, separators=(",", ":")).encode()).hexdigest()

    def _validated_dataset(self) -> tuple[list[Path], Path, AggregateTradeManifest | MonthlyAggregateTradeManifest, str]:
        ...
        try:
            ...
            manifest = MonthlyAggregateTradeManifest.model_validate(raw) if "partitions" in raw else AggregateTradeManifest.model_validate(raw)
        ...
        parquets = [self.manifest_path.parent / item.file_name for item in manifest.partitions] if isinstance(manifest, MonthlyAggregateTradeManifest) else [self.manifest_path.with_name("aggregate_trades.parquet")]
        total_rows = 0
        ...
        if total_rows != manifest.row_count:
            raise ValueError(f"ValueAreaTrap row count mismatch: manifest={manifest.row_count}, parquet={total_rows}")
        dataset_directory_hash = self.manifest_path.parent.name
        ...
        parquet_hash = hashlib.sha256("".join(file_hash(item) for item in parquets).encode()).hexdigest()
        return parquets, self.manifest_path, manifest, parquet_hash
    ...
    def data_availability(self, specification: StrategySpec) -> list[DataAvailability]:
        parquets, manifest_path, manifest, parquet_hash = self._validated_dataset()
        ...
        return [DataAvailability(
            ...
            source_symbol=f"{symbol} perpetual",
            ...
            warnings=[("BTCUSDT perpetual is a crypto proxy and is not native CME MBT evidence" if symbol == "BTCUSDT" else f"{symbol} Binance perpetual is proxy evidence and is not native CME evidence"), f"manifest_path={manifest_path}", f"partition_set_hash={parquet_hash}"],
            declared_substitution=f"{symbol} perpetual aggregate trades -> no CME contract substitution",
        )]
    ...
    def _profile(
        ...
        *,
        ...
        while cumulative / total < value_area_fraction and (lower in volumes or upper in volumes):
        ...
        profile_payload = {"session": str(day), "bucket_size": str(bucket_size), "volumes": payload, "source_dataset_hash": source_hash}
        if session_definition != "US_CASH_WINDOW_PROXY":
        ...
        return SessionProfile(session_date=day, poc=poc, vah=max(chosen) + bucket_size, val=min(chosen), total_session_volume=total, value_area_volume=cumulative, coverage_ratio=cumulative / total, bucket_size=bucket_size, profile_hash=profile_hash, source_dataset_hash=source_hash, session_label=session_definition)

    def _stream_features(self, parquet: Path | list[Path], source_hash: str, config: ValueAreaTrapConfig, bucket_size: Decimal) -> tuple[list[FiveMinuteBar], dict[date, SessionProfile], dict[str, Any]]:
        if isinstance(parquet, list):
            bars: list[FiveMinuteBar] = []
            profiles: dict[date, SessionProfile] = {}
            totals = {"session_count": 0, "profile_count": 0, "bar_count": 0, "session_trade_rows": 0}
            for partition in parquet:
                partition_bars, partition_profiles, statistics = self._stream_features(partition, source_hash, config, bucket_size)
                overlap = set(profiles).intersection(partition_profiles)
                if overlap:
                    raise ValueError(f"monthly partitions contain overlapping UTC profile dates: {sorted(overlap)}")
                bars.extend(partition_bars)
                profiles.update(partition_profiles)
                for key, value in statistics.items():
                    totals[key] += value
            return sorted(bars, key=lambda item: item.start_utc), profiles, totals
        ...
        for batch in parquet_file.iter_batches(batch_size=100_000, columns=columns, use_threads=True):
            ...
            if config.session_definition == UTC_SESSION_LABEL:
                session_time = timestamps.dt.tz_convert(UTC)
                mask = pd.Series(True, index=frame.index)
            else:
                session_time = local
                mask = (local.dt.time >= time(9, 30)) & (local.dt.time < time(16, 0))
                if config.session_definition == US_CASH_SESSION_LABEL:
                    cash_day_mask = pd.Series(
                        [is_us_cash_trading_day(day) for day in local.dt.date],
                        index=frame.index,
                    )
                    mask = mask & cash_day_mask
            ...
            selected["session_date"] = session_time.loc[mask].dt.date.to_numpy()
        ...
        for start_utc, raw in sorted(bar_rows.items()):
            ...
            bars.append(FiveMinuteBar(start_utc=start_utc, end_utc=raw["end_utc"], start_new_york=raw["start_new_york"], end_new_york=raw["end_new_york"], open=raw["open"], high=raw["high"], low=raw["low"], close=raw["close"], total_volume=raw["total_volume"], aggressive_buy_volume=raw["aggressive_buy_volume"], aggressive_sell_volume=raw["aggressive_sell_volume"], bar_delta=raw["bar_delta"], cumulative_volume_delta=cvd_by_day[day], trade_count=raw["trade_count"], vwap=raw["vwap_numerator"] / raw["total_volume"], session_date=day, session_label=config.session_definition))
        profiles = {day: self._profile(day, {key: (value[0], value[1]) for key, value in buckets.items()}, bucket_size, source_hash, value_area_fraction=config.value_area_fraction, session_definition=config.session_definition) for day, buckets in profile_rows.items() if buckets}
    ...
    @staticmethod
    def _completed_us_cash_sessions(
        bars: list[FiveMinuteBar],
        profiles: dict[date, SessionProfile],
        split: SplitDefinition,
    ) -> tuple[list[FiveMinuteBar], dict[date, SessionProfile], dict[str, Any]]:
        """Keep only complete, pinned NYSE cash-session windows.

        Weekend and the three declared May--July 2026 full-day NYSE holidays
        are excluded before bars are built.  A remaining day is usable only if
        its observed five-minute bars cover both the 09:30 opening boundary and
        the 16:00 closing boundary in ``America/New_York``.
        """

        start_day = split.start_timestamp.astimezone(NY).date()
        end_day = split.end_timestamp.astimezone(NY).date()
        expected: list[date] = []
        current = start_day
        while current <= end_day:
            if is_us_cash_trading_day(current):
                expected.append(current)
            current += timedelta(days=1)
        by_day: dict[date, list[FiveMinuteBar]] = {}
        for bar in bars:
            by_day.setdefault(bar.session_date, []).append(bar)
        complete: set[date] = set()
        missing: list[dict[str, Any]] = []
        for day in expected:
            session_bars = sorted(by_day.get(day, []), key=lambda item: item.start_utc)
            if not session_bars:
                missing.append({"session_date": str(day), "reason": "NO_US_CASH_SESSION_TRADES"})
                continue
            first, last = session_bars[0], session_bars[-1]
            first_time = first.start_new_york.timetz().replace(tzinfo=None)
            last_time = last.end_new_york.timetz().replace(tzinfo=None)
            if first_time != time(9, 30) or last_time != time(16, 0):
                missing.append({
                    "session_date": str(day),
                    "reason": "INCOMPLETE_US_CASH_SESSION_BARS",
                    "first_bar_start": first.start_new_york.isoformat(),
                    "last_bar_end": last.end_new_york.isoformat(),
                })
                continue
            complete.add(day)
        filtered_bars = [item for item in bars if item.session_date in complete]
        filtered_profiles = {day: profile for day, profile in profiles.items() if day in complete}
        excluded_weekends = sum(
            1
        ...
        holidays = [
            str(day)
            for day in sorted(US_CASH_SESSION_HOLIDAYS_2026)
            if start_day <= day <= end_day
        ]
        return filtered_bars, filtered_profiles, {
            "timezone": "America/New_York",
            "session_open": "09:30",
            "session_close": "16:00",
            "dst_aware": True,
            "eligible_session_count": len(expected),
            "complete_session_count": len(complete),
            "excluded_weekend_count": excluded_weekends,
            "excluded_exchange_holidays": holidays,
    ...
    def _diagnostics(self, result, metrics: dict[str, Any], availability: list[DataAvailability], trades_path: Path, output_dir: Path, spec: StrategySpec) -> Path:
        ...
        diagnostics = {
            ...
            "session_boundary": (
                {"terminal_flatten_cluster": False, "terminal_flatten": True, "timezone": "America/New_York", "dst_documented": True}
                if metrics.get("session_definition", "US_CASH_WINDOW_PROXY") == "US_CASH_WINDOW_PROXY"
                else {
                    "terminal_flatten_cluster": False,
                    "terminal_flatten": True,
                    "session_definition": metrics["session_definition"],
                    "timezone": metrics["session_timezone"],
                    "dst_documented": metrics["session_definition"] == US_CASH_SESSION_LABEL,
                    **({"us_cash_session_diagnostics": metrics["us_cash_session_diagnostics"]} if "us_cash_session_diagnostics" in metrics else {}),
                }
            ),
            ...
            "data_sources": [{"provider": item.provider, "source_symbol": item.source_symbol, "native_or_proxy": "proxy", "synthetic_transformation": "none", "dataset_hash": item.dataset_hash, **({"evidence_label": metrics["evidence_label"]} if metrics.get("session_definition") == UTC_SESSION_LABEL else {})} for item in availability],
    ...
    def _run(self, spec: StrategySpec, split: SplitDefinition, phase: str, parameters: dict[str, Any], output_dir: Path, experiment_id: str) -> ResearchArtifact:
        ...
        if str(values.get("symbol", manifest.symbol)).upper() != manifest.symbol.upper():
            raise ValueError("ValueAreaTrap specification symbol does not match its immutable manifest")
        config = ValueAreaTrapConfig(**{key: value for key, value in values.items() if key in ValueAreaTrapConfig.model_fields})
        bars, profiles, feature_stats = self._stream_features(parquets, manifest.normalized_dataset_hash, config, Decimal(str(values.get("price_bucket_size", "10"))))
        ...
        if config.session_definition == US_CASH_SESSION_LABEL:
            bars, profiles, session_diagnostics = self._completed_us_cash_sessions(bars, profiles, split)
        selected_sessions = {item.session_date for item in bars}
        selected_profiles = {item.session_date for item in profiles.values() if item.session_date in selected_sessions}
        ...
        zero_reason = (
            "No completed US cash sessions were available after weekend, holiday, and session-boundary validation."
            if not result.trades and config.session_definition == US_CASH_SESSION_LABEL and not selected_sessions
            else "No qualifying stop-run/CVD-divergence/return setup was produced by the real state machine in the requested split."
            if not result.trades
            else None
        )
        net_values = [Decimal(item["net_pnl"]) for item in result.trades]
        cumulative = Decimal(); peak = Decimal(); maximum_drawdown = Decimal()
        for value in net_values:
            cumulative += value; peak = max(peak, cumulative); maximum_drawdown = max(maximum_drawdown, peak - cumulative)
        net_profit = sum((value for value in net_values if value > 0), Decimal())
        net_loss = -sum((value for value in net_values if value < 0), Decimal())
        profit_factor = None if net_loss == 0 else float(net_profit / net_loss)
        total_costs = result.fees + result.slippage_cost
        metrics = {"execution_mode": "REAL_DATA", "provider": "Binance USD-M Futures", "symbol": manifest.symbol, "manifest_path": str(manifest_path), "parquet_paths": [str(item) for item in parquets], "partition_set_hash": parquet_hash, "dataset_hash": manifest.normalized_dataset_hash, "row_count": manifest.row_count, "source_date_start": str(manifest.date_start), "source_date_end": str(manifest.date_end), "session_count": len(selected_sessions), "profile_count": len(selected_profiles), "five_minute_bar_count": len(bars), "proposed_setups": result.proposed_setups, "significant_stop_runs": result.significant_stop_runs, "confirmed_divergences": result.confirmed_divergences, "return_triggers": result.return_triggers, "compliance_blocks": len(result.compliance_blocks), "executed_trades": len(result.trades), "blocked_trades": len(result.compliance_blocks), "wins": sum(Decimal(item["net_pnl"]) > 0 for item in result.trades), "losses": sum(Decimal(item["net_pnl"]) <= 0 for item in result.trades), "gross_pnl": float(result.gross_pnl), "fees": float(result.fees), "slippage": float(result.slippage_cost), "net_pnl": float(result.net_pnl), "policy_hash": result.policy_hash, "execution_cost_configuration_hash": result.cost_model_hash, "alpha_evaluation": alpha_evaluation.model_dump(mode="json"), "alpha_qualified": alpha_qualified.model_dump(mode="json"), "zero_trade_reason": zero_reason, "transferability_warning": ("BTCUSDT Binance perpetual is a crypto proxy and is not native CME MBT." if manifest.symbol == "BTCUSDT" else f"{manifest.symbol} Binance perpetual is proxy evidence and is not native CME or Alpha Futures performance.")}
        if values.get("study_label"):
            metrics.update({
                "study_label": values["study_label"],
                "selection_prohibited": bool(values.get("selection_prohibited")),
                "optimization_claimed": bool(values.get("optimization_claimed")),
                "confirmation_evidence": bool(values.get("confirmation_evidence")),
                "requires_future_holdout": bool(values.get("requires_future_holdout")),
            })
        if config.session_definition in {UTC_SESSION_LABEL, US_CASH_SESSION_LABEL}:
            metrics.update({"session_definition": config.session_definition, "session_timezone": config.session_timezone, "evidence_label": f"Binance {manifest.symbol} perpetual proxy evidence; not CME or Alpha Futures performance.", "volume_qualified_stop_runs": result.significant_stop_runs, "total_costs": float(total_costs), "average_trade": float(result.net_pnl / len(result.trades)) if result.trades else 0.0, "profit_factor": profit_factor, "maximum_drawdown": float(maximum_drawdown)})
        if session_diagnostics is not None:
            metrics["us_cash_session_diagnostics"] = session_diagnostics
        input_path.write_text(json.dumps({"strategy_id": spec.strategy_id, "specification_hash": spec.specification_hash, "execution_mode": "REAL_DATA", "manifest_path": str(manifest_path), "parquet_paths": [str(item) for item in parquets], "partition_set_hash": parquet_hash, "dataset_hash": manifest.normalized_dataset_hash, "split_hash": split.split_hash, "parameters": values, "phase": phase}, indent=2, sort_keys=True, default=str), encoding="utf-8")
        metrics_path.write_text(json.dumps(metrics, indent=2, sort_keys=True), encoding="utf-8")
        try:
            report_payload = build_value_area_reports(result, bars, profiles, config)
        except (TypeError, ValueError) as exc:
            report_payload = {"raw_strategy": raw_strategy_metrics(result), "alpha_zero_25k_evaluation": alpha_evaluation.model_dump(mode="json"), "alpha_zero_25k_qualified": alpha_qualified.model_dump(mode="json"), "warnings": [f"optional fixed-ablation diagnostics unavailable: {exc}", f"{manifest.symbol} is not MES, MNQ, ES, or NQ.", "No report establishes CME performance or Alpha Futures compliance."]}
        if values.get("study_label"):
