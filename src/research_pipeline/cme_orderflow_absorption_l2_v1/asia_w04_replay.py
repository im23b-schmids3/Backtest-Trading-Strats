    ...
    *, repository_root: Path,
    ...
    if session_dates is None:
        ...
        if len(requested) != len(set(requested)):
            raise AsiaReplayError("requested Asia session dates contain duplicates")
        by_day = {row.day: row for row in all_sessions}
        missing = sorted(set(requested) - set(by_day))
        if missing:
            raise AsiaReplayError(f"requested Asia session dates are not audited: {missing}")
        ...
        if unknown_profile_only:
            raise AsiaReplayError(f"requested Asia profile-only dates are not audited: {unknown_profile_only}")
        if profile_only & set(requested):
            raise AsiaReplayError("a requested Asia strategy date cannot also be profile-only")
        ...
        if ineligible:
            raise AsiaReplayError(f"requested Asia session dates are not replay-compatible: {ineligible}")
        selected_days = set(requested)
        # Include only the causal profile predecessor(s) needed to seed the
        # first selected day; they remain profile-only context in the output.
        ...
        while frontier:
            ...
            if prior and prior not in selected_days:
                if prior not in by_day:
                    raise AsiaReplayError(f"requested Asia session lacks audited prior profile: {current.day}")
