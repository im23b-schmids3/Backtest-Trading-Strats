...
def load_audit_sessions(audit_root: Path) -> tuple[AuditSession, ...]:
    ...
    if excluded != list(summary.get("candidate_session_universe", {}).get("missing_prior_europe_poc_dates", ())):
    ...
    if any(row.source_model != "MBO_DERIVED_MBP10" for row in rows):
        raise EuropeReplayError("Europe baseline must use only the audited MBO-derived public view")
    return tuple(rows)
