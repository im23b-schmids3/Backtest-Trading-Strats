"""Deterministic Europe-session replay for the frozen W04 candidate.
...
STRATEGY_ID = "CMEOrderflowAbsorption.ES_L2_W04_EUROPE_POC_MES_PROXY"
CONFIG_ID = "W04-02-06-04-04-Q45"
EVIDENCE_LABEL = "EUROPE_W04_ES_NATIVE_MES_PROXY_RETROSPECTIVE_RESEARCH"
INTERPRETATION = "RETROSPECTIVE_RESEARCH_NOT_FRESH_OOS_EVIDENCE"
DISCLAIMER = (
    "THIS IS RETROSPECTIVE EUROPE RESEARCH USING ES-BASED MES PROXY EXECUTION. "
    "IT IS NOT NATIVE MES EXECUTION AND NOT FRESH OOS EVIDENCE."
)
EUROPE_LEVEL_NAME = "PRIOR_EUROPE_SESSION_POC"
EUROPE_TIMEZONE = "Europe/London"
EUROPE_START_LOCAL = time(8, 0)
EUROPE_SIGNAL_END_LOCAL = time(16, 30)
EUROPE_CUTOFF_LOCAL = time(16, 55)
EUROPE_HARD_FLAT_REASON = "HARD_FLAT_EUROPE_1655_LONDON"
EUROPE_ENTRY_CUTOFF_REASON = "SESSION_ENTRY_CUTOFF_1655_LONDON"
_GENERIC_HARD_FLAT_UNRESOLVED = frozenset({
    "UNRESOLVED_NO_ENTRY_OBSERVATION",
    "UNRESOLVED_NO_LATER_EVENT",
    "UNRESOLVED_AT_HARD_FLAT",
})
EUROPE_LIQUIDATION_LOOKBACK_NS = 1_000_000_000
EXPECTED_ELIGIBLE_SESSIONS = 46
EXPECTED_SOURCE_SESSIONS = 48
ELIGIBLE_CLASSIFICATION = "MISSING_MES_EXECUTION"
PROFILE_ONLY_CLASSIFICATION = "MISSING_PRIOR_EUROPE_POC"
OUTPUT_ROOT = Path("research_runs/CMEOrderflowAbsorption.ES_L2_W04_EUROPE_POC_MES_PROXY")
AUDIT_ROOT = Path("research_runs/CMEOrderflowAbsorption.ES_L2_EUROPE_DATA_COVERAGE_AUDIT")
...
class EuropeReplayError(RuntimeError):
    """The isolated Europe replay cannot satisfy its frozen contract."""


@dataclass(frozen=True)
class EuropeStructuralLevel:
    ...
    def __post_init__(self) -> None:
        if self.name != EUROPE_LEVEL_NAME:
            raise EuropeReplayError("Europe replay accepts only PRIOR_EUROPE_SESSION_POC")
        value = float(self.price)
        if not 0.0 < value < 100_000.0 or not math.isclose(
            value / TICK, round(value / TICK), rel_tol=0.0, abs_tol=1e-9,
        ):
            raise EuropeReplayError("Europe POC must be a normalized ES tick price")
...
def _read_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise EuropeReplayError(f"missing or invalid JSON: {path}") from exc
    if not isinstance(value, dict):
        raise EuropeReplayError(f"JSON artifact is not an object: {path}")
    return value
...
def europe_clock_ns(day: str, local_time: time) -> int:
    """Resolve a sealed Europe/London wall-clock boundary to UTC nanoseconds."""
    local = datetime.combine(datetime.fromisoformat(day).date(), local_time, tzinfo=ZoneInfo(EUROPE_TIMEZONE))
    return int(local.astimezone(UTC).timestamp() * 1_000_000_000)


def europe_session_bounds(day: str) -> dict[str, int]:
    start = europe_clock_ns(day, EUROPE_START_LOCAL)
    signal_end = europe_clock_ns(day, EUROPE_SIGNAL_END_LOCAL)
    cutoff = europe_clock_ns(day, EUROPE_CUTOFF_LOCAL)
    if not start < signal_end < cutoff:
        raise EuropeReplayError(f"invalid Europe/London session boundaries: {day}")
    return {"start_ns": start, "signal_end_ns": signal_end, "cutoff_ns": cutoff}
...
def in_europe_window(day: str, timestamp_ns: int) -> bool:
    bounds = europe_session_bounds(day)
    return bounds["start_ns"] <= int(timestamp_ns) < bounds["signal_end_ns"]


def prior_europe_poc(volume_by_tick: Mapping[int, int]) -> float:
    ...
    if not valid:
        raise EuropeReplayError("completed Europe session has no ES executions for POC construction")
...
def _execution_tick(execution: Execution) -> int:
    ...
    if not math.isclose(value, rounded, rel_tol=0.0, abs_tol=1e-9):
        raise EuropeReplayError("ES execution is not aligned to the frozen 0.25-point tick")
    return int(rounded)
...
def frozen_semantic_contracts() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    europe = {
        **unchanged,
        ...
        "session_window": "[08:00:00,16:30:00)_Europe/London",
        "structural_level": EUROPE_LEVEL_NAME,
        "position_management_cutoff": "16:55:00_Europe/London_LAST_VALID_BBO_IN_INCLUSIVE_PRECEDING_1S",
...
def semantic_diff_document() -> dict[str, Any]:
    ny, europe = frozen_semantic_contracts()
    differences = {
        key: {"ny_w04": ny.get(key), "europe_w04": europe.get(key)}
        for key in sorted(set(ny) | set(europe))
        if ny.get(key) != europe.get(key)
    }
    ...
    if unexpected or missing:
        raise EuropeReplayError(
            f"Europe semantic isolation failed; unexpected={unexpected}, missing={missing}"
        )
    return {
        "status": "PASS",
        "reference": CONFIG_ID,
        "europe_strategy_id": STRATEGY_ID,
        ...
        "unchanged_contract_sha256": _canonical_sha256({
            key: value for key, value in europe.items() if key not in ALLOWED_SEMANTIC_DIFFERENCES
        }),
        "ny_contract_sha256": _canonical_sha256(ny),
        "europe_contract_sha256": _canonical_sha256(europe),
    }


def build_europe_coverage_audit(repository_root: Path, audit_root: Path) -> dict[str, Any]:
    """Materialize a metadata-only Europe coverage audit from the sealed local inventory.

    The existing Asia audit is used only as a complete inventory/index of local
    artifacts and candidate dates.  Europe eligibility is reclassified from
    the declared source windows before any strategy outcome is opened.
    """
    source_root = repository_root / "research_runs/CMEOrderflowAbsorption.ES_L2_ASIA_DATA_COVERAGE_AUDIT"
    source_summary = _read_json(source_root / "summary.json")
    if source_summary.get("market_data_opened") is not False or source_summary.get("strategy_replay_executed") is not False:
        raise EuropeReplayError("source inventory provenance is not metadata-only")
    with (source_root / "session-coverage.csv").open(newline="", encoding="utf-8") as handle:
        source_sessions = list(csv.DictReader(handle))
    if len(source_sessions) != 98:
        raise EuropeReplayError("local candidate-session inventory is incomplete")

    session_rows: list[dict[str, Any]] = []
    eligible_dates: list[str] = []
    missing_profile_dates: list[str] = []
    for source in source_sessions:
        day = str(source["session_date"])
        original = str(source["classification"])
        bounds = europe_session_bounds(day)
        if original == "MISSING_PRIOR_ASIA_POC":
            classification = PROFILE_ONLY_CLASSIFICATION
            missing_profile_dates.append(day)
            reason = "full ES MBO coverage exists but the immediately preceding completed Europe profile is unavailable"
        elif original == "MISSING_MES_EXECUTION":
            classification = ELIGIBLE_CLASSIFICATION
            eligible_dates.append(day)
            reason = "full Europe ES MBO coverage and prior profile are available; native MES is absent so labeled proxy is required"
        else:
            classification = "PARTIAL_SESSION"
            reason = "native MBP-10 begins at 13:00 UTC and omits the canonical Europe/London session open"
        prior = str(source.get("prior_asia_session") or "")
        if prior == "NOT_EVALUATED":
            prior = ""
        session_rows.append({
            "session_date": day,
            "period": str(source["period"]),
            "source_model": str(source["source_model"]),
            "europe_start_utc": _iso_ns(bounds["start_ns"]),
            "europe_signal_end_utc": _iso_ns(bounds["signal_end_ns"]),
            "europe_cutoff_utc": _iso_ns(bounds["cutoff_ns"]),
            "es_europe_coverage": "FULL" if classification in {ELIGIBLE_CLASSIFICATION, PROFILE_ONLY_CLASSIFICATION} else "PARTIAL",
            "depth_initialization": str(source.get("depth_initialization") or "NOT_EVALUATED"),
            "prior_europe_session": prior,
            "prior_europe_profile_available": "YES" if classification == ELIGIBLE_CLASSIFICATION else "NO",
            "native_mes_europe_coverage": "NONE",
            "execution_model": "MES_PROXY_FROM_ES" if classification == ELIGIBLE_CLASSIFICATION else "NOT_EXECUTED",
            "technically_eligible": classification == ELIGIBLE_CLASSIFICATION,
            "classification": classification,
            "reason": reason,
        })

    with (source_root / "data-inventory.csv").open(newline="", encoding="utf-8") as handle:
        source_inventory = list(csv.DictReader(handle))
    inventory_rows: list[dict[str, Any]] = []
    for row in source_inventory:
        source_class = str(row["source_classification"])
        native_signal = source_class == "NATIVE_MBP10"
        identity = f"{row['inventory_id']} {row['path']}".upper()
        symbol = "MES" if "MES" in identity and "ES_MES" not in identity else "ES" if any(token in identity for token in ("ES_", "ES/", "MBO", "PILOT", "OOS")) else "MULTI_OR_METADATA"
        schema = (
            "MBO" if "MBO" in identity else "MBP-10" if "MBP10" in identity
            else "MBP-1" if "MBP1" in identity else "TRADES" if "TRADE" in identity
            else "PARQUET_CAUSAL_TAPE" if "PARQUET" in str(row["artifact_type"]) else "MANIFEST_OR_REPORT"
        )
        mbo_source = source_class == "MBO_DERIVED_MBP10"
        inventory_rows.append({
            ...
            "path": str(row["path"]),
            "manifest_or_index_sha256": str(row["manifest_or_index_sha256"]),
            "file_count": str(row["file_count"]),
            "total_bytes": str(row["total_bytes"]),
            "symbol": symbol,
            "schema": schema,
            "declared_coverage_utc": str(row["declared_coverage_utc"]),
            "earliest_utc": f"DECLARED_GROUP_RANGE:{row['declared_coverage_utc']}",
            "latest_utc": f"DECLARED_GROUP_RANGE:{row['declared_coverage_utc']}",
            "trading_session_date": f"MULTI_SESSION:{row['period']}",
            "complete_or_partial": "COMPLETE_FOR_EUROPE_SOURCE_SESSIONS" if mbo_source else "PARTIAL_OR_PROVENANCE_ONLY",
            "usable_for_europe_signal_reconstruction": mbo_source,
            "usable_for_prior_europe_profile": mbo_source,
            "usable_for_execution_tail": mbo_source or native_signal,
            "europe_session_relevance": "PARTIAL_SESSION" if native_signal else "PROVENANCE_OR_MBO_SOURCE",
            "reuse_decision": "REBUILD_FROM_HASH_BOUND_MBO" if source_class == "MBO_DERIVED_MBP10" else "NOT_FULL_EUROPE_SIGNAL_SOURCE",
            "notes": str(row["notes"]).replace("Asia", "Europe").replace("asia", "Europe"),
        })

    counts = Counter(str(row["classification"]) for row in session_rows)
    summary = {
        "audit_id": "CMEOrderflowAbsorption.ES_L2_EUROPE_DATA_COVERAGE_AUDIT",
        "status": "EUROPE_TRANSFER_DATA_READY_WITH_MES_PROXY",
        "scope": "READ_ONLY_LOCAL_DATA_INVENTORY_AND_PRE_OUTCOME_SESSION_CONTRACT",
        "market_data_opened": False,
        "strategy_replay_executed": False,
        "strategy_outcomes_inspected": False,
        "pnl_calculated": False,
        "network_market_data_calls": 0,
        "canonical_europe_session": {
            "timezone": EUROPE_TIMEZONE,
            "start_inclusive": EUROPE_START_LOCAL.isoformat(),
            "signal_end_exclusive": EUROPE_SIGNAL_END_LOCAL.isoformat(),
            "position_management_cutoff": EUROPE_CUTOFF_LOCAL.isoformat(),
            "summer_utc": {"signal": "07:00:00-15:30:00", "cutoff": "15:55:00"},
            "winter_utc": {"signal": "08:00:00-16:30:00", "cutoff": "16:55:00"},
            "cutoff_basis": "25 deterministic minutes after the London cash close and five minutes inside the shortest sealed 16:00 UTC source boundary",
            "dst_rule": "IANA Europe/London",
            "cme_maintenance_overlap": False,
            "profile_rule": "executed ES trades in the immediately preceding completed canonical Europe session; lower POC tie and canonical 70% value area",
            "cash_session_anchor": "London Stock Exchange regular trading 08:00-16:30 local",
        },
        "descriptive_alternatives_not_backtested": [
            "Xetra 09:00-17:30 Europe/Berlin",
            "broader London/continental-Europe overlap windows",
        ],
        "candidate_session_universe": {
            "total": len(session_rows),
            "classification_counts": dict(sorted(counts.items())),
            "missing_prior_europe_poc_dates": missing_profile_dates,
            "es_and_prior_poc_present_but_mes_missing_dates": eligible_dates,
            "eligible_with_mes_proxy_count": len(eligible_dates),
        },
        "causal_tape_reuse": {
            "reusable_without_rebuild": False,
            "decision": "REBUILD_FROM_HASH_BOUND_RAW_SOURCE",
            "reason": "existing NY/Asia tapes do not contain the sealed Europe session contract and level identities",
        },
        "public_session_sources": [
            {
                "title": "London Stock Exchange technical trading-hours documentation",
                "url": "https://docs.londonstockexchange.com/sites/default/files/documents/servicetechnicaldescriptionintroductionofnewtradingcurrencies.pdf",
                "fact_used": "Regular trading is 08:00-16:30 London local time.",
            },
            {
                "title": "Deutsche Boerse Xetra trading hours",
                "url": "https://www.cashmarket.deutsche-boerse.com/cash-en/trading/trading-calendar-and-trading-hours",
                "fact_used": "Xetra 09:00-17:30 CET/CEST is retained as a descriptive alternative only.",
            },
            {
                "title": "CME E-mini trading-hours FAQ",
                "url": "https://www.cmegroup.com/articles/faqs/e-mini-s-p-500-tuesday-and-thursday-options-frequently-asked-questions.html",
                "fact_used": "The daily CME maintenance interval does not overlap the sealed Europe window.",
            },
        ],
    }
    audit_root.mkdir(parents=True, exist_ok=True)
    _write_json(audit_root / "summary.json", summary)
    _write_csv(audit_root / "session-coverage.csv", session_rows)
    _write_csv(audit_root / "data-inventory.csv", inventory_rows)
    _write_csv(
        audit_root / "eligible-sessions.csv",
        [row for row in session_rows if bool(row["technically_eligible"])],
    )
    return summary
...
def load_audit_sessions(audit_root: Path, *, include_native: bool = False) -> tuple[AuditSession, ...]:
    ...
    if summary.get("audit_id") != "CMEOrderflowAbsorption.ES_L2_EUROPE_DATA_COVERAGE_AUDIT":
        raise EuropeReplayError("wrong Europe coverage-audit identity")
    if summary.get("market_data_opened") is not False or summary.get("strategy_replay_executed") is not False:
        raise EuropeReplayError("Europe coverage audit provenance is not read-only")
        ...
        with (audit_root / "session-coverage.csv").open(newline="", encoding="utf-8") as handle:
            for row in csv.DictReader(handle):
                ...
                rows.append(AuditSession(
                    ...
                    str(row["prior_europe_session"]) or None, classification,
                ))
    except OSError as exc:
        raise EuropeReplayError("Europe session coverage CSV is unavailable") from exc
    ...
    if len(rows) != expected_source_count or len(eligible) != expected_eligible_count:
        raise EuropeReplayError(
            f"Europe audit chronology mismatch: source={len(rows)}, eligible={len(eligible)}"
        )
    if len(days) != len(set(days)) or days != sorted(days):
        raise EuropeReplayError("Europe source chronology is duplicate or unordered")
    excluded = [row.day for row in rows if not row.eligible]
    expected_excluded = list(summary.get("candidate_session_universe", {}).get("missing_prior_europe_poc_dates", ()))
    if native_dec_jan:
        expected_excluded.append(native_dec_jan[0].day)
    if excluded != sorted(expected_excluded):
        raise EuropeReplayError("profile-only dates disagree with the sealed coverage audit")
    ...
    if [row.day for row in eligible] != expected_eligible:
        raise EuropeReplayError("eligible Europe sessions disagree with the sealed coverage audit")
    if any(row.source_model not in public_books.SUPPORTED_SOURCE_MODELS for row in rows):
        raise EuropeReplayError("Europe manifest declares an unsupported public-book source model")
    return tuple(rows)
...
def _may_binding(repository_root: Path, sessions: Sequence[AuditSession]) -> list[SourceBinding]:
    ...
    if manifest.get("data_acquired") is not True or not isinstance(files, dict):
        raise EuropeReplayError("sealed May acquisition manifest is invalid")
    ...
    for session in sessions:
        ...
        if not isinstance(record, dict) or record.get("schema") != "mbo" or record.get("session_date") != session.day:
            raise EuropeReplayError(f"May ES MBO manifest binding is invalid: {session.day}")
...
def _retro_binding(repository_root: Path, sessions: Sequence[AuditSession]) -> list[SourceBinding]:
    ...
    for session in sessions:
        ...
        if not isinstance(record, dict):
            raise EuropeReplayError(f"retro ES MBO manifest binding is missing: {session.day}")
...
def _dec_jan_native_binding(repository_root: Path, sessions: Sequence[AuditSession]) -> list[SourceBinding]:
    ...
    if not isinstance(pre_files, dict) or not isinstance(post_files, dict):
        raise EuropeReplayError("native Dec/Jan source manifests are invalid")
    output: list[SourceBinding] = []
    for session in sessions:
        ...
        if not isinstance(pre, dict) or not isinstance(post, dict) or post_key is None:
            raise EuropeReplayError(f"native Dec/Jan MBP-10 binding is missing: {session.day}")
        output.append(SourceBinding(
...
def verify_source_bindings(bindings: Sequence[SourceBinding]) -> list[dict[str, Any]]:
    ...
    for index, binding in enumerate(bindings, start=1):
        if not binding.path.is_file():
            raise EuropeReplayError(f"sealed local MBO source is missing: {binding.path}")
        actual_bytes = binding.path.stat().st_size
        if binding.expected_bytes is not None and actual_bytes != binding.expected_bytes:
            raise EuropeReplayError(f"sealed local MBO size mismatch: {binding.path}")
        print(
            f"EUROPE_SOURCE_VERIFY {index:02d}/{len(bindings):02d} {binding.path.name}",
            flush=True,
        )
        actual_sha256 = _sha256(binding.path)
        if actual_sha256.lower() != binding.expected_sha256.lower():
            raise EuropeReplayError(f"sealed local MBO SHA-256 mismatch: {binding.path}")
        if len(binding.extra_paths) != len(binding.extra_sha256) or len(binding.extra_paths) != len(binding.extra_bytes):
            raise EuropeReplayError("native source binding hash metadata is incomplete")
        for extra_path, extra_hash, extra_size in zip(binding.extra_paths, binding.extra_sha256, binding.extra_bytes):
            if not extra_path.is_file() or extra_path.stat().st_size != extra_size or _sha256(extra_path).lower() != extra_hash.lower():
                raise EuropeReplayError(f"sealed native source hash/size mismatch: {extra_path}")
        verified.append({
...
def _protected_ny_snapshot(repository_root: Path) -> dict[str, str]:
    ...
    if missing:
        raise EuropeReplayError(f"protected NY W04 evidence is missing: {missing}")
...
def _interaction_row(interaction: L2Interaction, day: str) -> dict[str, Any]:
    if interaction.end_ns is None or interaction.end_price is None:
        raise EuropeReplayError("only completed Europe interactions can be summarized")
    if interaction.level.name != EUROPE_LEVEL_NAME:
        raise EuropeReplayError("RTH structural-level leakage detected in Europe interaction")
...
class EuropeProxySessionCausalTape(corrected.BerlinSessionCausalTape):
    ...
    def __init__(self, day: str, rows: Iterable[Mapping[str, Any]]) -> None:
        ...
        cutoff_ns = europe_session_bounds(day)["cutoff_ns"]
        if (
            ...
            raise EuropeReplayError(f"Europe causal tape lacks its exact 16:55 London terminal: {day}")
        self.hard_flat_timestamp_ns = cutoff_ns
        self.hard_flat_utc = datetime.fromtimestamp(cutoff_ns / 1e9, tz=UTC)
        self.hard_flat_local = self.hard_flat_utc.astimezone(ZoneInfo(EUROPE_TIMEZONE))
        self.contract_terminal_kind = "HARD_FLAT_EUROPE_1655_LONDON"
        ...
        self.hard_event = {
            **self.original_terminal_event,
            ...
            "hard_flat_reason": EUROPE_HARD_FLAT_REASON,
    ...
    def _terminal_candidate(
        ...
        if observation.reason != "HARD_FLAT_BERLIN":
            return observation
        # Unlike the NY tapes, the Europe source is deliberately consumed only
    ...
    def entry_outcome(self, interaction: Mapping[str, Any], event_ordinal: int) -> matrix.EntryOutcome:
        ...
        if native_instrument not in {"ES", "MES"}:
            raise EuropeReplayError(f"unexpected frozen sizing instrument: {native_instrument}")
        ...
        trade["setup_id"] = f"EUROPE:{interaction['interaction_id']}"
        trade["trade_id"] = f"EUROPE_T:{interaction['interaction_id']}"
...
def _classify_europe_entry_cutoff(
    session: matrix.SessionResult, *, tape: matrix.SessionCausalTape,
) -> None:
    """Turn generic hard-flat waiting states into the sealed Europe cutoff outcome.
    ...
    entry observation before its terminal event.  A complete Europe tape has an
    explicit 16:55 London hard-flat event, so those states are deterministically
    blocked by the sealed no-entry-at-or-after-cutoff rule; they are not source
    ...
    for identifier in identifiers:
        session.terminal_outcomes[identifier] = EUROPE_ENTRY_CUTOFF_REASON
    count = len(identifiers)
    if count > session.unresolved:
        raise EuropeReplayError("Europe cutoff classification exceeds unresolved setup count")
    session.unresolved -= count
    session.other_terminal[EUROPE_ENTRY_CUTOFF_REASON] = (
        session.other_terminal.get(EUROPE_ENTRY_CUTOFF_REASON, 0) + count
    )


def observe_execution_with_session_gate(
    engine: L2InteractionEngine, execution: Execution, *, signal_end_ns: int,
) -> None:
    """Advance active lifecycles after signal end without opening a new one."""
    if execution.timestamp_ns < signal_end_ns:
        engine.observe_execution(execution)
        return
...
class _EuropeMboState:
    def __init__(self, spec: SessionSpec, prior_poc: float | None) -> None:
        ...
        if spec.eligible:
            if prior_poc is None:
                raise EuropeReplayError(f"eligible Europe session lacks prior POC: {spec.day}")
            level = EuropeStructuralLevel(EUROPE_LEVEL_NAME, prior_poc)
    ...
    def _append(
        ...
        event_spec = master.SessionBuildSpec(
            ...
            EUROPE_HARD_FLAT_REASON, str(self.spec.staging_root), "EUROPE_LONDON",
        )
    ...
    def observe(self, record: object) -> None:
        if self.closed:
            raise EuropeReplayError(f"event routed to closed Europe session: {self.spec.day}")
        self.source_index += 1
        ...
        if quote is None:
            raise EuropeReplayError(f"MBO adapter emitted non-executable Europe BBO: {self.spec.day}")
        ...
        if public.execution is not None:
            observe_execution_with_session_gate(
                self.engine, public.execution, signal_end_ns=self.spec.signal_end_ns,
            )
            self._drain()
    ...
    def finish(self) -> dict[str, Any]:
        if self.closed:
            raise EuropeReplayError(f"duplicate Europe session close: {self.spec.day}")
        if not self.reached_cutoff:
            raise EuropeReplayError(f"source did not reach the 16:55 London cutoff: {self.spec.day}")
        self.adapter.finish()
        session_poc = prior_europe_poc(self.profile_volume)
        result: dict[str, Any] = {
            ...
            "prior_europe_poc": self.prior_poc,
        ...
        if quote is None or quote_ns is None:
            raise EuropeReplayError(
                f"16:55 London hard flat lacks an ES BBO in the preceding inclusive second: {self.spec.day}"
            )
        ...
        if len(indexes) != len(self.interactions):
            raise EuropeReplayError(f"Europe interaction/index mismatch: {self.spec.day}")
        index_by_id = {str(row["interaction_id"]): row for row in indexes}
        accepted = [row for row in self.interactions if bool(row["accepted"])]
        tape = EuropeProxySessionCausalTape.from_parquet(self.spec.day, Path(event_artifact["path"]))
        session = matrix.simulate_independent_session(tape, accepted, index_by_id)
        _classify_europe_entry_cutoff(session, tape=tape)
        if session.unresolved != 0:
            ...
            raise EuropeReplayError(
                f"unresolved Europe setup/trade at session close: {self.spec.day}: "
                f"{dict(sorted(unresolved.items()))}"
            )
        if len(session.terminal_outcomes) != len(accepted):
            raise EuropeReplayError(f"accepted setup terminal mismatch: {self.spec.day}")
        for trade in session.trades:
            if float(trade["estimated_initial_risk_usd"]) > RISK_BUDGET_USD + 1e-9:
                raise EuropeReplayError(f"risk-budget overrun: {trade['trade_id']}")
            if int(trade["contracts"]) < 1:
                raise EuropeReplayError(f"zero-size executed trade: {trade['trade_id']}")
...
def _load_checkpoint(staging: Path, session: AuditSession) -> dict[str, Any] | None:
    ...
    if (
        payload.get("status") != "EUROPE_SESSION_COMPLETE"
        ...
        raise EuropeReplayError(f"stale or incompatible Europe checkpoint: {session.day}")
...
def _save_checkpoint(staging: Path, result: Mapping[str, Any]) -> None:
    ...
    _write_json(_session_checkpoint_path(staging, day), {
        "status": "EUROPE_SESSION_COMPLETE",
...
def _spec(session: AuditSession, binding: SourceBinding, staging: Path) -> SessionSpec:
    bounds = europe_session_bounds(session.day)
    return SessionSpec(
        session.day, session.period, session.source_model, session.prior_day,
        session.eligible, bounds["start_ns"], bounds["signal_end_ns"],
        bounds["cutoff_ns"], binding.path, staging,
    )
...
def _process_daily_binding(
    ...
    *, staging: Path,
    ...
    if checkpoint is not None:
        return checkpoint
    state = _EuropeMboState(_spec(session, binding, staging), _require_prior(session, previous_result))
    ...
    try:
        for record in public_books.stream_source(binding.path, session.source_model, binding.extra_paths):
            ...
            if state.decoded_records >= next_progress:
                print(
                    f"EUROPE_REPLAY {session.day} records={state.decoded_records:,} "
...
def _process_shared_binding(
    ...
    *, staging: Path,
    ...
    if not pending:
        return results
    states: dict[str, _EuropeMboState] = {}
    next_progress = 5_000_000
    records = 0
    try:
        ...
        for record in public_books.stream_source(binding.path, source_model, binding.extra_paths):
            ...
            if state is None:
                ...
                for earlier_session in earlier:
                    ...
                    if restored is not None:
                        prior = restored
                state = _EuropeMboState(_spec(session, binding, staging), _require_prior(session, prior))
                states[day] = state
            state.observe(record)
            records += 1
            if state.reached_cutoff:
            ...
            if records >= next_progress:
                print(
                    f"EUROPE_REPLAY {binding.period} records={records:,} "
                    f"completed={len(results):,}/{len(sessions):,}",
                    flush=True,
                )
                next_progress += 5_000_000
        if pending:
            raise EuropeReplayError(f"shared MBO source did not complete Europe sessions: {sorted(pending)}")
    except BaseException:
...
def _performance(trades: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    result.update({
        ...
        # The shared historical aggregator recognizes its frozen NY cutoff
        # label. Europe has a namespaced terminal reason for auditability.
        "hard_cutoff_exits": sum(
            str(row.get("exit_reason", "")) == EUROPE_HARD_FLAT_REASON
            for row in trades
        ),
    })
    return result
...
def _trade_and_setup_rows(
    ...
    for session in sessions:
        ...
        if set(terminals) != {str(row["interaction_id"]) for row in accepted}:
            raise EuropeReplayError(f"setup terminal universe mismatch: {session['session_date']}")
        for row in accepted:
            ...
            setup = {
                ...
                "prior_europe_poc": session["prior_europe_poc"],
    ...
    if len({row["setup_id"] for row in setups}) != len(setups):
        raise EuropeReplayError("duplicate Europe setup ID")
    if len({row["trade_id"] for row in trades}) != len(trades):
        raise EuropeReplayError("duplicate Europe trade ID")
    if any(row["level"] != EUROPE_LEVEL_NAME for row in setups):
        raise EuropeReplayError("RTH level leaked into setup ledger")
...
def _report_markdown(summary: Mapping[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        f"- Signal session: `[08:00:00, 16:30:00) {EUROPE_TIMEZONE}`; level: `{EUROPE_LEVEL_NAME}`.",
        ...
        "- Position-management cutoff: 16:55 Europe/London using the last executable ES BBO in the inclusive preceding one-second window.",
...
<h2>Contract</h2><p>08:00-16:30 Europe/London signal eligibility, 16:55 London management cutoff, prior-Europe POC only, W04 Q45, +3 tick causal confirmation, 2ms latency, 5-tick zone stop, 3R target, USD 250 fixed risk, one position.</p>
...
def _materialize(
    *, staging: Path,
    ...
    if not reconciliation["pass"]:
        raise EuropeReplayError("aggregate setup/trade reconciliation failed")
    ...
    if not risk["pass"]:
        raise EuropeReplayError("risk-budget audit failed")
    summary: dict[str, Any] = {
        "status": "EUROPE_W04_REPLAY_COMPLETE",
        "strategy_id": STRATEGY_ID,
        "config_id": CONFIG_ID,
        "evidence_label": EVIDENCE_LABEL,
        ...
        "session_contract": {
            ...
            "signal_end_exclusive": EUROPE_SIGNAL_END_LOCAL.isoformat(),
            "position_management_cutoff": EUROPE_CUTOFF_LOCAL.isoformat(),
            "reference_level": EUROPE_LEVEL_NAME,
        },
        "weights": {key: str(value) for key, value in W04_WEIGHTS.items()},
        "quality_threshold": str(QUALITY_THRESHOLD),
    ...
    eligible_rows = [{
        ...
        "prior_europe_session": row["prior_europe_session"],
        "prior_europe_poc": row["prior_europe_poc"],
    ...
    *, repository_root: Path,
    ...
    for binding in bindings:
        ...
        if binding.shared:
            ...
            print(
                f"EUROPE_SESSION {len(results) + 1:02d}/{EXPECTED_SOURCE_SESSIONS:02d} "
    ...
    if [str(row["session_date"]) for row in results] != [row.day for row in sessions]:
        raise EuropeReplayError("replay results do not match the audited source chronology")
