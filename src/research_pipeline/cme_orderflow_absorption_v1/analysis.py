...
VICINITY_TICKS, TIMEOUT_NS = 4, 60_000_000_000
...
class Diagnostics:
    ...
    active: dict[tuple[str, str, int], Interaction] = field(default_factory=dict)
    ...
    def _close(self, key: tuple[str, str, int], interaction: Interaction, reason: str, ts: int, price: int) -> None:
    ...
    def _resolve_responses(self, ts: int, price: int) -> None:
        while self.pending_responses and self.pending_responses[0][0] <= ts:
            ...
            if interaction.end_price is not None:
                interaction.responses[horizon] = (price - interaction.end_price) // TICK
    ...
    def _touch(self, day: str, ts: int, price: int, applied: Applied | None, spread: int | None) -> None:
        for key, interaction in list(self.active.items()):
            if key[0] != day: continue
            if ts - interaction.start_ns >= 0 and ts - getattr(interaction, "last_execution_ns", interaction.start_ns) > TIMEOUT_NS:
                self._close(key, interaction, "EXECUTION_TIMEOUT", ts, price)
            elif abs(price - interaction.level_price) > VICINITY_TICKS * TICK:
                self._close(key, interaction, "VICINITY_EXIT", ts, price)
        for name, level in self._levels_for(day, price).items():
            if abs(price - level) <= VICINITY_TICKS * TICK:
                ...
                interaction = self.active[key]; interaction.events += 1
                ...
                if applied.executed:
                    ...
                    interaction.last_execution_ns = ts
                    if applied.action == "F":
    ...
    def observe(self, rec: object, applied: Applied | None, spread: int | None = None) -> None:
        ...
        if RTH_START <= second < RTH_END:
            ...
            self._touch(day, rec.ts_recv, applied.price, applied, spread)
    ...
    def finalize(self) -> None:
        for key, interaction in list(self.active.items()): self._close(key, interaction, "RTH_END", interaction.end_ns or interaction.start_ns, interaction.level_price)
