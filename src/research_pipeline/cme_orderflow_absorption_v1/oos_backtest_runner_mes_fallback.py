"""Seen-data research runner: V1 signal with deterministic ES -> MES sizing fallback.

This is NOT the sealed original V1 result. It reuses the sealed V1 signal and
execution semantics, but adds a position-sizing fallback: prefer ES; when one ES
exceeds the $250 risk budget, use the maximum whole MES contracts that fit.
MES execution is a proxy from the ES MBO price path in this research file.
"""
...
RUN_ROOT = ROOT / "research_runs/CMEOrderflowAbsorption.ES_V1_MES_FALLBACK_RESEARCH/seen_aug"
...
# HIER deine echte MES-Kommission pro Contract / pro Seite einsetzen.
# Nicht einfach die ES-Kommission übernehmen.
...
def size_trade_with_mes_fallback(p: dict[str, float]) -> dict[str, float | int | str]:
    # ES bleibt immer erste Wahl.
    es = size_trade(p)
    ...
    # Fail closed, solange die echte MES-Gebühr nicht festgelegt wurde.
    if MES_COMMISSION_PER_SIDE is None:
        raise SealedRunError("MES commission per side must be frozen before MES fallback")
    ...
    mes_price_risk = mes_raw + mes_slip
    ...
    mes_contracts = math.floor(RISK_BUDGET / mes_initial_risk)

    return {
        "raw_price_risk_usd": mes_raw,
        "slippage_contribution_usd": mes_slip,
        "one_contract_price_risk_usd": mes_price_risk,
...
def _audit(state:State,row:dict[str,Any],outcome:str)->None:
 ...
 elif outcome in {"INSUFFICIENT_RISK_BUDGET_FOR_ONE_ES_CONTRACT","INSUFFICIENT_RISK_BUDGET_FOR_ES_AND_MES"}:state.counters["insufficient_risk_budget"]+=1
...
def write_outputs(out:Path,source_sha:str,state:State)->None:
 ...
 (out/"run-manifest.json").write_bytes(canonical({"immutable":True,"research_only":True,"interpretation":"SEEN_AUG_DATA_NOT_FRESH_OOS_EVIDENCE","mes_execution_model":"MES_PROXY_EXECUTION_FROM_ES_MBO","mes_commission_per_side":MES_COMMISSION_PER_SIDE,"contract_hashes":contract_hashes(),"source_sha256":source_sha,"legacy_empty_result_invalid":True,"score_calibration_source":"DEVELOPMENT_ONLY","oos_rank_recomputation_count":0}))
...
def main(argv:list[str]|None=None)->int:
 ...
 for rec in DBNStore.from_file(args.dbn):
  ...
  if position and rec.ts_recv > position["cutoff_ns"]:
   ...
   ts,q=cutoff_quote; exit_ref=q[0] if position["direction"]=="LONG" else q[1]; exit_fill=exit_ref-TICK if position["direction"]=="LONG" else exit_ref+TICK; gross=(exit_fill-position["entry"])*(1 if position["direction"]=="LONG" else -1)*position["usd_per_point"]*position["contracts"]; fees=2*position["commission_per_side"]*position["contracts"]
  ...
  while pending and pending[0]["interaction_end"]+2_000_000<=rec.ts_recv:
   ...
   if not s["contracts"]:_audit(state,row,"INSUFFICIENT_RISK_BUDGET_FOR_ES_AND_MES");continue
  ...
  if position and quote:
   ...
   if hit:
    exit_ref=bid if position["direction"]=="LONG" else ask;exit_fill=exit_ref-TICK if position["direction"]=="LONG" else exit_ref+TICK;reason="STOP" if ((position["direction"]=="LONG" and bid<=position["stop"]) or (position["direction"]=="SHORT" and ask>=position["stop"])) else "TARGET";gross=(exit_fill-position["entry"])*(1 if position["direction"]=="LONG" else -1)*position["usd_per_point"]*position["contracts"];fees=2*position["commission_per_side"]*position["contracts"];state.trades.append({**position,"exit_timestamp":rec.ts_recv,"exit_fill":exit_fill,"exit_reason":reason,"gross_usd":gross,"commission_usd":fees,"net_usd":gross-fees,"r_multiple":gross/(position["one_contract_initial_risk_usd"]*position["contracts"])});state.counters["closed_trades"]+=1;_audit(state,position,"ENTERED_"+reason);position=None
