...
from .manifests import require_development_mode,verify_manifest,ManifestError
...
def run_candidate(bars:list[Bar], candidate:Candidate, assumptions:ExecutionAssumptions=ExecutionAssumptions())->dict:
 ...
 for index,bar in enumerate(bars):
  # Closed-bar confirmation can only activate on following bar.
  ...
  for order in list(pending):
   ...
   if active is not None:
    reason="ACTIVE_POSITION_BLOCKED"
   if reason:
   ...
   if reject:
    outcomes.append({"setup_id":order["setup_id"],"disposition":reject});pending.remove(order);continue
   if trade:
...
def development_diagnostic(*,eth_manifest:str|Path,btc_manifest:str|Path)->dict: return {"eth":verify_manifest(eth_manifest,mode="development"),"btc":verify_manifest(btc_manifest,mode="development"),"holdout_status":"LOCKED_NOT_OPENED"}
def run_development(**kwargs):
 # Deliberately explicit future entry point; no implicit input discovery.
 # This validates the invocation boundary only; execution remains unavailable
 # until an authorized development runner is supplied.
 ...
 if not artifact_root.is_absolute():
  raise ValueError("FIB09_V1_ARTIFACT_ROOT_MUST_BE_ABSOLUTE")
 if artifact_root.exists():
  raise FileExistsError("FIB09_V1_IMMUTABLE_ARTIFACT_ROOT_COLLISION")
 if not repository_root.is_absolute() or not repository_root.is_dir():
  raise ValueError("FIB09_V1_REPOSITORY_ROOT_MUST_BE_ABSOLUTE_EXISTING")
 return development_diagnostic(eth_manifest=kwargs["eth_manifest"], btc_manifest=kwargs["btc_manifest"])
