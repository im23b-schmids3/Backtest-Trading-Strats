...
from .models import DATASET_HASH, SOURCE_MANIFEST_HASH

STRATEGY_ID = "ImbalanceVWAPRide.BTC_MACRO_BINS_V2_EXPLORATORY"
ADAPTER_ID = "imbalance-vwap-ride-btc-macro-bins-v2"
EVIDENCE_LABEL = "POST_HOC_V2_DEVELOPMENT"
SPEC_VERSION = "imbalance-vwap-ride-btc-macro-bins-v2-exploratory-1"
COST_MODEL_VERSION = "binance-btcusdt-verified-research-costs-v2"


class ImbalanceVWAPRideV2Config(StrictModel):
    variant_id: str = "baseline"
    bin_size_usd: Decimal = Field(default=Decimal("50"), gt=0)
    min_imbalance_ratio: Decimal = Field(default=Decimal("3"), gt=1)
    ...
    zone_expiry_bars: int = Field(default=36, ge=1)
    ...
    def sealed_invariants(self) -> "ImbalanceVWAPRideV2Config":
...
BASELINE = ImbalanceVWAPRideV2Config()
...
def preregistered_variants() -> list[ImbalanceVWAPRideV2Config]:
    """Return the baseline plus the exact stable 17-setting V2 OAT registry."""

    variants = [BASELINE]
    baseline_payload = BASELINE.model_dump(mode="python")
    for name, values in PARAMETER_REGISTRY:
        for value in values:
            if value == baseline_payload[name]:
                continue
    ...
    if len(variants) != 18 or len(set(identities)) != len(identities):
        raise AssertionError("V2 one-factor registry is not exact and unique")
...
def _metric_gt(metrics: dict[str, Any], name: str, threshold: float) -> bool:
    try:
...
def _long_short_reconciles(metrics: dict[str, Any]) -> bool:
    return bool(metrics.get("long_short_reconciliation", {}).get("reconciles")) and (
        int(metrics.get("long_trades", -1)) + int(metrics.get("short_trades", -1))
        == int(metrics.get("executed_trades", -2))
    )
...
def development_gate(metrics: dict[str, Any]) -> dict[str, Any]:
    checks = {
        ...
        "net_profit_factor": _metric_gt(metrics, "net_profit_factor", 1.05),
        ...
        "long_short_reporting": _long_short_reconciles(metrics),
    }
    return {
        "passed": all(checks.values()),
        "checks": checks,
        "edge_diagnosis": diagnose_development_failure(metrics),
    }


def diagnose_development_failure(metrics: dict[str, Any]) -> dict[str, Any]:
    enough_trades = int(metrics.get("executed_trades", 0)) >= 40
    gross_edge = (
        _metric_gt(metrics, "gross_profit_factor", 1.05)
        and _finite_metric(metrics, "average_gross_r")
        and float(metrics["average_gross_r"]) > 0
    )
    net_edge = (
        _metric_gt(metrics, "net_profit_factor", 1.05)
        and _finite_metric(metrics, "average_net_r")
        and float(metrics["average_net_r"]) > 0
    )
    if not enough_trades:
        classification = "RESTRICTIVE_THRESHOLD_SAMPLE_INSUFFICIENCY"
    elif not gross_edge:
        classification = "PRE_COST_EDGE_FAILURE"
    elif not net_edge:
        classification = "COST_DESTROYED_EDGE"
    else:
        classification = "EDGE_SURVIVES_COSTS"
    return {
...
def validation_gate(metrics: dict[str, Any]) -> dict[str, Any]:
    checks = {
        ...
        "net_profit_factor": _metric_gt(metrics, "net_profit_factor", 1),
        "average_net_r": _finite_metric(metrics, "average_net_r") and float(metrics["average_net_r"]) > 0,
...
def baseline_distance(config: ImbalanceVWAPRideV2Config) -> int:
    return sum(
...
def freeze_development_candidates(
    candidates: list[tuple[ImbalanceVWAPRideV2Config, dict[str, Any]]],
) -> list[tuple[ImbalanceVWAPRideV2Config, dict[str, Any]]]:
...
def select_validation_candidate(
    candidates: list[tuple[ImbalanceVWAPRideV2Config, dict[str, Any]]],
) -> tuple[ImbalanceVWAPRideV2Config, dict[str, Any]] | None:
    passing = [item for item in candidates if validation_gate(item[1])["passed"]]
    if not passing:
        return None

    def key(item: tuple[ImbalanceVWAPRideV2Config, dict[str, Any]]) -> tuple[Any, ...]:
        config, metrics = item
        return (
