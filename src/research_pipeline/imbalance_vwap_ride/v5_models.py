...
def candidate_configuration_hash(config: ImbalanceVWAPRideV5Config) -> str: return sha256_value(config.frozen_payload())
...
def phase_b_gate(metrics):
    ms=_months(metrics,PHASE_B_MONTHS); n=lambda m:int(ms[m].get("executed_trades",0)); pnls=[sum((_d(ms[m].get("net_pnl",0)) for m in g),Decimal()) for g in (PHASE_B_MONTHS[:3],PHASE_B_MONTHS[3:])]; full=_d(metrics.get("net_pnl",0)); pos=[max(_d(x.get("net_pnl",0)),Decimal()) for x in ms.values()]; total=sum(pos,Decimal()); checks={"minimum_24_trades":int(metrics.get("executed_trades",0))>=24,"five_active_months":sum(n(m)>0 for m in ms)>=5,"four_months_with_three_trades":sum(n(m)>=3 for m in ms)>=4,"net_pnl_positive":full>0,"net_pf_above_1_05":_d(metrics.get("net_profit_factor",0))>Decimal("1.05"),"average_net_r_positive":_d(metrics.get("average_net_r",0))>0,"two_target_hits":int(metrics.get("target_hits",0))>=2,"mfe_1r_15pct":_d(metrics.get("mfe_at_least_1r_rate",0))>=Decimal(".15"),"integrity":_common(metrics) and bool(metrics.get("hashes_valid")) and bool(metrics.get("costs_valid")),"best_month_70pct":not total or max(pos)/total<=Decimal(".7"),"best_five_75pct":_d(metrics.get("best_five_positive_pnl_contribution",1))<Decimal(".75"),"subperiods":any(x>0 for x in pnls) and all(x>=-(full/2) for x in pnls)}; return {"passed":all(checks.values()),"status":"LOCKED_TEST_PASSED" if all(checks.values()) else "LOCKED_TEST_FAILED","checks":checks}

def alpha_eligible(locked_phase_b: dict[str, Any], metrics: dict[str, Any], *, phase_b_execution_count: int) -> bool: return bool(locked_phase_b.get("passed")) and phase_b_execution_count == 1 and int(metrics.get("executed_trades",0)) >= 24
