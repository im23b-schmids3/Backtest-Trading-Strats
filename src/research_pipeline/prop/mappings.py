...
def default_market_mappings() -> list[MarketMapping]:
    return [
        MarketMapping(strategy_market="BTCUSDT", source_symbol="BTCUSDT", source_provider="declared_strategy_source", target_futures_contract="MBT", mapping_method="native symbol family", native_or_proxy="native", reference_price_method="close", duplicate_exposure_group="crypto_btc", date_range={"start": "2019-01-01", "end": "open"}, confidence_level=ConfidenceClass.NATIVE_FUTURES_SUPPORTED, limitations=["continuous-contract construction must be verified"]),
